...
The RPM spec file provided here is for Chellit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://chellit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Chellit git repository](https://github.com/ChellitNetwork/ChellitNetwork) but are not part of
...
Currently only `Source10` is used, for the example `chellit.conf` file.
...
in the chellit git repository. At present only an SVG version of the Chellit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the chellit
...
needed when 0.12.1 is released, a proper fix is already in the Chellit git
master branch.
...
idea to build Chellit Core against a static version of that library compiled
...
The `chellit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `chellit-server` package. At this point, most RPM based
...
The `chellit-server` package should have SELinux support. How to properly do
...
Chellit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/chellit/). He has been
packaging Chellit for Fedora at least since 2012.
...
4. I renamed the `chellit` package that contains the Qt GUI to `chellit-core` as
that appears to be how the general population refers to it, in contrast to
`chellit-xt` or `chellit-classic`. I wanted to make sure the general population
