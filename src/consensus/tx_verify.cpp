...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, CTokensCache* tokenCache, bool fCheckDuplicateInputs, bool fMemPoolCheck, bool fCheckTokenDuplicate, bool fForceDuplicateCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        // Make sure that all token tx have a nValue of zero CHE
    ...
    if (AreTokensDeployed()) {
        if (tokenCache) {
            if (tx.IsNewToken()) {
                ...
                // above transaction types.  Also fail if it contains OP_CHE_TOKEN opcode but wasn't a valid script.
                for (auto out : tx.vout) {
                    int nType;
                    bool _isOwner;
                    if (out.scriptPubKey.IsTokenScript(nType, _isOwner)) {
                        ...
                        if (out.scriptPubKey.Find(OP_CHE_TOKEN) > 0) {
