...
const std::string ChellitGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ChellitGUI::DEFAULT_WALLET = "~Default";

double GetPoSKernelPS();

ChellitGUI::ChellitGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...

{
    ...
    // MacDockIconHandler::instance()->setIcon(QIcon(":icons/about_che"));
#endif
...
void ChellitGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to an Chellit address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and chellit: URIs)"));
    ...
    transferTokenAction->setStatusTip(tr("Transfer tokens to CHE addresses"));
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about_che"), tr("&About %1").arg(tr(PACKAGE_NAME)), this);
    ...
    toggleHideAction = new QAction(platformStyle->TextColorIcon(":/icons/about_che"), tr("&Show / Hide"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Chellit addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Chellit addresses"));
    ...
    openAction->setStatusTip(tr("Open a chellit: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Chellit command-line options").arg(tr(PACKAGE_NAME)));
...
void ChellitGUI::createToolBars()
{
...
void ChellitGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ChellitGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ChellitGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ChellitGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ChellitGUI::setWalletActionsEnabled(bool enabled)
{
...
void ChellitGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ChellitGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ChellitGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ChellitGUI::optionsClicked()
{
...
void ChellitGUI::showDebugWindow()
{
...
void ChellitGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ChellitGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ChellitGUI::openClicked()
{
...
void ChellitGUI::gotoOverviewPage()
{
    footerWidget->show();
    overviewAction->setChecked(true);
...
void ChellitGUI::gotoHistoryPage()
{
    footerWidget->hide();
...
void ChellitGUI::gotoReceiveCoinsPage()
{
    footerWidget->hide();
...
void ChellitGUI::gotoSendCoinsPage(QString addr)
{
    footerWidget->hide();
...
void ChellitGUI::gotoSignMessageTab(QString addr)
{
    footerWidget->hide();
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ChellitGUI::gotoVerifyMessageTab(QString addr)
{
    footerWidget->hide();
...
/** TOKENS START */
void ChellitGUI::gotoTokensPage()
{
    footerWidget->hide();
...
void ChellitGUI::gotoCreateTokensPage()
{
    footerWidget->hide();
...
void ChellitGUI::gotoManageTokensPage()
{
    footerWidget->hide();
...
void ChellitGUI::updateNetworkState()
{
    ...
    case 1: case 2: case 3: icon = ":/icons/connectboost::placeholders::_1"; break;
    case 4: case 5: case 6: icon = ":/icons/connectboost::placeholders::_2"; break;
    case 7: case 8: case 9: icon = ":/icons/connectboost::placeholders::_3"; break;
    default: icon = ":/icons/connectboost::placeholders::_4"; break;
    }

    QString tooltip;

    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Chellit network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ChellitGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ChellitGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ChellitGUI::updateHeadersSyncProgressLabel()
{
...
void ChellitGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ChellitGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Chellit"); // default title
...
void ChellitGUI::changeEvent(QEvent *e)
{
...
void ChellitGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ChellitGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ChellitGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& tokenName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (tokenName == "CHE")
        msg += tr("Amount: %1\n").arg(ChellitUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ChellitUnits::formatWithCustomName(tokenName, amount, MAX_TOKEN_UNITS, true));
...
void ChellitGUI::checkTokens()
{
    ...
    if(AreTokensDeployed()) {
        ...
        transferTokenAction->setVisible(true);
        createTokenAction->setVisible(true);
        manageTokenAction->setVisible(true);
    } else {
        ...
        transferTokenAction->setVisible(false);
        createTokenAction->setVisible(false);
        manageTokenAction->setVisible(false);
    }
}
#endif // ENABLE_WALLET

void ChellitGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ChellitGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ChellitGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ChellitGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ChellitGUI::setHDStatus(int hdEnabled)
{
...
void ChellitGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ChellitGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ChellitGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ChellitGUI::updateStakingIcon()
{
    if (walletFrame) {
...
void ChellitGUI::detectShutdown()
{
...
void ChellitGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ChellitGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ChellitGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ChellitGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ChellitGUI::subscribeToCoreSignals()
{
...
void ChellitGUI::unsubscribeFromCoreSignals()
{
...
void ChellitGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ChellitGUI::toggleStakingActive()
{
    ...
    menu(0)
{
    ...
    QList<ChellitUnits::Unit> units = ChellitUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ChellitUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(ChellitUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (ChellitUnits::Unit u : ChellitUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ChellitUnits::name(u)), this);
...
{
    setText(ChellitUnits::name(newUnits));
}
