...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** TOKENS START */
    case OP_CHE_TOKEN              : return "OP_CHE_TOKEN";
    /** TOKENS END */
    ...
    if (this->size() > 30) {
        if ((*this)[25] == OP_CHE_TOKEN) { // OP_CHE_TOKEN is always in the 25 index of the script if it exists
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == CHE_T) { // Transfer first anticipating more transfers than other tokens operations
                    nType = TX_TRANSFER_TOKEN;
                    return true;
                } else if ((*this)[index] == CHE_Q && this->size() > 39) {
                    nType = TX_NEW_TOKEN;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == CHE_O) {
                    nType = TX_NEW_TOKEN;
                    fIsOwner = true;
                    return true;
