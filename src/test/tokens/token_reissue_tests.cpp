...
BOOST_FIXTURE_TEST_SUITE(token_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test)
    {
        ...
        CNewToken token1("CHETOKEN", CAmount(100 * COIN), 8, 1, 0, "");

        // Add an token to a valid CHE address
        ...
        CReissueToken reissue1("CHETOKEN", CAmount(1 * COIN), 8, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("CHETOKEN"), "Map Reissued Token should contain the token \"CHETOKEN\"");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("CHETOKEN", Params().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");

        // Get the new token data from the cache
        CNewToken token2;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("CHETOKEN", token2), "Failed to get the token2");
        ...
        BOOST_CHECK_MESSAGE(token2.strName == "CHETOKEN", "Token2: Token name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("CHETOKEN", CBlockTokenUndo{true, false, "", 0}));
        ...
        CNewToken token3;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("CHETOKEN", token3), "Failed to get the token3");
        ...
        BOOST_CHECK_MESSAGE(token3.strName == "CHETOKEN", "Token3: Token name is wrong");
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("CHETOKEN"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("CHETOKEN", Params().GlobalBurnAddress())) == CAmount(100 * COIN), "Tokens total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CReissueToken reissue3("CHETOKEN", CAmount(1 * COIN), 7, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueToken reissue4("CHETOKEN", CAmount(1 * COIN), -1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));

        BOOST_CHECK_MESSAGE(reissue4.IsValid(error, cache), "Reissue4 wasn't valid");

        // Create a new token object with units of 0
        CNewToken token2("CHETOKEN2", CAmount(100 * COIN), 0, 1, 0, "");

        // Add new token to a valid CHE address
        ...
        CReissueToken reissue5("CHETOKEN2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueToken reissue6("CHETOKEN2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
