...
BOOST_FIXTURE_TEST_SUITE(token_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of CHELLITCOIN NOT allowed
        BOOST_CHECK(!IsTokenNameValid("CHE", type));
        BOOST_CHECK(!IsTokenNameValid("CHELLIT", type));
        BOOST_CHECK(!IsTokenNameValid("CHELLITCOIN", type));

        //- Versions of CHELLITCOIN ALLOWED
        BOOST_CHECK(IsTokenNameValid("CHELLIT.COIN", type));
        BOOST_CHECK(IsTokenNameValid("CHELLIT_COIN", type));
        BOOST_CHECK(IsTokenNameValid("CHESPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDERCHE", type));
        BOOST_CHECK(IsTokenNameValid("CHELLITSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDECHELLIT", type));
        BOOST_CHECK(IsTokenNameValid("BLACK_CHELLITS", type));
        BOOST_CHECK(IsTokenNameValid("SECHEOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_token_coin_test)
    {
        ...
        CTokenTransfer token("CHELLIT", 1000, 0);
    ...
    BOOST_AUTO_TEST_CASE(new_token_coin_test)
    {
        ...
        // Create the token scriptPubKey
        CNewToken token("CHELLIT", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        token.ConstructTransaction(scriptPubKey);
