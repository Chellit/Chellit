...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapTokenCoins, bool fGetCHE, bool fGetTokens, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth, bool fLockedTokens) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetCHE) { // Looking for CHE Tx OutPoints Only
                    if (fCHELimitHit) // We hit our limit
                        continue;

                    // We only want CHE OutPoints. Don't include Token OutPoints
                    if (isTokenScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListTokens() const
{
    ...
    // https://github.com/ChellitNetwork/ChellitNetwork/pull/10340, but that change has been
    // postponed until after https://github.com/ChellitNetwork/ChellitNetwork/pull/10244 to
                                   ...
                                   const KnownTokenType& tokenType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-chellit-address
 ...
 * https://chellittalk.org/?topic=54527, or
 * https://github.com/ChellitNetwork/ChellitNetwork/pull/1393.
 */
