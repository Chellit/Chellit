...
`test/functional/test_framework/test_framework.py:ChellitTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/chellit_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_chellitd = config["components"].getboolean("ENABLE_CHELLITD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/ChellitNetwork/ChellitNetwork/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/ChellitNetwork/ChellitNetwork/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_chellitd):
        print("No functional tests to run. Wallet, utils, and chellitd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if chellitd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "chellitd"]) is not None:
            print("%sWARNING!%s There is already a chellitd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CHELLITD" not in os.environ:
        os.environ["CHELLITD"] = build_dir + '/src/chellitd' + exeext
        os.environ["CHELLITCLI"] = build_dir + '/src/chellit-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `chellit-cli help` (`rpc_interface.txt`).
